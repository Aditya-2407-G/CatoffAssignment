...
import { userCreatedGames } from '@/lib/appwrite';
import { useNavigation } from '@react-navigation/native';
import { useGlobalContext } from '@/context/GlobalContextProvider';
import { Loader } from '@/components/Loader';

const UserCreatedGamesScreen = () => {
    ...
    useEffect(() => {
        const fetchUserGames = async () => {
            try {
                ...
                const res = userCreatedGames(userId);
    ...
    const renderGameItem = ({ item }: any) => (
        <SafeAreaView className="m-2 p-5 bg-white rounded-lg shadow-md">
            ...
            {/* @ts-ignore */}
            <Button mode="contained" onPress={() => navigation.navigate('GameDetailScreen', { game: item })}>
                View Details
            </Button>
        </SafeAreaView>
    );
    ...
    return (
        <SafeAreaView className="flex-1 bg-gray-100 p-2">
            <Text className="text-2xl font-bold text-center my-5">Your Created Games - {userGames.length}</Text>
            <FlatList
                data={userGames}
...
export default UserCreatedGamesScreen;
